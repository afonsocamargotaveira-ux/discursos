...
def gerar_docx(resultados, termo_busca, textos_integrais):
    ...
    doc.add_paragraph(f"Termo de busca: {termo_busca.strip() or '(nenhum — todos os discursos do período)'}")
...
with st.sidebar:
    ...
    termo_busca = st.text_input(
        "Termo / assunto do discurso (opcional)",
        placeholder="ex: educação, meio ambiente, reforma tributária...",
        help="Deixe em branco para recuperar TODOS os discursos do(s) "
        "deputado(s) e período selecionados, sem filtrar por assunto.",
    )
...
if "docx_buffer" in st.session_state:
    st.download_button(
        ...
        file_name=f"discursos_{(termo_busca.strip() or 'todos').replace(' ', '_')}.docx",
